...
require_once $_SERVER['DOCUMENT_ROOT'] . '/model/curse_tag_model.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/vendor/autoload.php';

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;

class Curse_tag extends Common
{
   private $curse_tag_model;
    public function __construct(Curse_tag_model $curse_tag_model,Account_model $account_model){
        parent::__construct($account_model);
        parent::init();
        $this->curse_tag_model = $curse_tag_model;
    }
    ...
    {
        $list = $this->curse_tag_model->getList();
        return $list;
    }

    public function get($id){
        $info = $this->curse_tag_model->get($id);
        return $info;
    }
    ...
    private function create($data) {
        ...
        $result = $this->curse_tag_model->create($name,$status,$editor);
        if($result===SUCCESS){
    ...
    private function edit($data) {
        ...
        $name = $data['name'];
        $status = intval($data['status']);
        $checkExist = $this->curse_tag_model->get($id);
        if(empty($checkExist)){
            $response = json_encode(['errCode'=>CURSE_CATEGORY_NOT_EXIST]);
            echo $response;
        }
        $result = $this->curse_tag_model->edit($id,$name,$status);
        if($result===SUCCESS){
            $response = json_encode(['errCode'=>SUCCESS,'redirect'=>'list.php']);
           echo $response;
           exit;
    ...
    private function delete($data) {
        $id = intval($data['id']);
        $result = $this->curse_tag_model->delete($id);
        if($result===SUCCESS){
            $response = json_encode(['errCode'=>SUCCESS,'redirect'=>'list.php']);
           echo $response;
           exit;
      ...
      public function export($format)
    {
        ...
        $list = $this->curse_tag_model->getExportList();
        switch ($format) {
            case 1:
                header('Content-Type: text/csv; charset=utf-8');
                header('Content-Disposition: attachment; filename=curse_tag.csv');
            ...
            case 2:
                ...
                header('Content-Disposition:attachment;filename="curse_tag.xlsx"');
...
$curse_tag = new Curse_tag(new Curse_tag_model($db),new Account_model($db));

if (in_array('export', $query_array)) {
    $curse_tag->export($query_array[1]);
    exit;
}

$curse_tag->requestEntry();
