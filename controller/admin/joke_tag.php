...
require_once $_SERVER['DOCUMENT_ROOT'] . '/model/joke_tag_model.php';
require_once $_SERVER['DOCUMENT_ROOT'] . '/vendor/autoload.php';

use PhpOffice\PhpSpreadsheet\Spreadsheet;
use PhpOffice\PhpSpreadsheet\Writer\Xlsx;

class Joke_tag extends Common
{
   private $joke_tag_model;
    public function __construct(Joke_tag_model $joke_tag_model,Account_model $account_model){
        parent::__construct($account_model);
        parent::init();
        $this->joke_tag_model = $joke_tag_model;
    }
    ...
    {
        $list = $this->joke_tag_model->getList();
        return $list;
    }

    public function get($id){
        $info = $this->joke_tag_model->get($id);
        return $info;
    }
    ...
    private function edit($data) {
        ...
        $name = $data['name'];
        $status = intval($data['status']);
        $checkExist = $this->joke_tag_model->get($id);
        if(empty($checkExist)){
            $response = json_encode(['errCode'=>CURSE_CATEGORY_NOT_EXIST]);
            echo $response;
        }
        $result = $this->joke_tag_model->edit($id,$name,$status);
        if($result===SUCCESS){
            $response = json_encode(['errCode'=>SUCCESS,'redirect'=>'list.php']);
           echo $response;
           exit;
    ...
    private function delete($data) {
        $id = intval($data['id']);
        $result = $this->joke_tag_model->delete($id);
        if($result===SUCCESS){
            $response = json_encode(['errCode'=>SUCCESS,'redirect'=>'list.php']);
           echo $response;
           exit;
...
$joke_tag = new Joke_tag(new Joke_tag_model($db),new Account_model($db));

if (in_array('export', $query_array)) {
    $joke_tag->export($query_array[1]);
    exit;
}

$joke_tag->requestEntry();
