...
class Touching_source extends Common
{
    private $touching_source_model;
    public function __construct(Touching_source_model $touching_source_model, Account_model $account_model)
    {
    ...
    {
        $list = $this->touching_source_model->getList();
        return $list;
    }

    public function get($id)
    {
        $info = $this->touching_source_model->get($id);
        return $info;
    }
    ...
    private function edit($data)
    {
        ...
        $result = $this->touching_source_model->edit($id, $name, $status);
        if ($result === SUCCESS) {
            $response = json_encode(['errCode' => SUCCESS, 'redirect' => 'list.php']);
            echo $response;
            exit;
...
$touching_source = new Touching_source(new Touching_source_model($db), new Account_model($db));

if (in_array('export', $query_array)) {
    $touching_source->export($query_array[1]);
    exit;
}
