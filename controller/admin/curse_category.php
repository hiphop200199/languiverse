...
class Curse_category extends Common
{
   private $curse_category_model;
    public function __construct(Curse_category_model $curse_category_model,Account_model $account_model){
    ...
    {
        $list = $this->curse_category_model->getList();
        return $list;
    }

    public function get($id){
        $info = $this->curse_category_model->get($id);
        return $info;
    }
    ...
    private function create($data) {
        ...
        $result = $this->curse_category_model->create($name,$status,$editor);
        if($result===SUCCESS){
    ...
    private function edit($data) {
        ...
        $name = $data['name'];
        $status = intval($data['status']);
        $checkExist = $this->curse_category_model->get($id);
        if(empty($checkExist)){
            $response = json_encode(['errCode'=>CURSE_CATEGORY_NOT_EXIST]);
            echo $response;
        }
        $result = $this->curse_category_model->edit($id,$name,$status);
        if($result===SUCCESS){
            $response = json_encode(['errCode'=>SUCCESS,'redirect'=>'list.php']);
           echo $response;
           exit;
    ...
    private function delete($data) {
        $id = intval($data['id']);
        $result = $this->curse_category_model->delete($id);
        if($result===SUCCESS){
            $response = json_encode(['errCode'=>SUCCESS,'redirect'=>'list.php']);
           echo $response;
           exit;
   ...
   public function export($format)
    {
        ...
        $list = $this->curse_category_model->getExportList();
        switch ($format) {
            case 1:
                header('Content-Type: text/csv; charset=utf-8');
                header('Content-Disposition: attachment; filename=curse_category.csv');
            ...
            case 2:
                ...
                header('Content-Disposition:attachment;filename="curse_category.xlsx"');
...
$curse_category = new Curse_category(new Curse_category_model($db),new Account_model($db));

if (in_array('export', $query_array)) {
    $curse_category->export($query_array[1]);
    exit;
}

$curse_category->requestEntry();

unset($curse_category);