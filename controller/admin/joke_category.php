...
class Joke_category extends Common
{
    private $joke_category_model;
    public function __construct(Joke_category_model $joke_category_model, Account_model $account_model)
    {
    ...
    {
        $list = $this->joke_category_model->getList();
        return $list;
    }

    public function get($id)
    {
        $info = $this->joke_category_model->get($id);
        return $info;
    }
    ...
    private function create()
    {
        ...
        $result = $this->joke_category_model->create($name, $status, $editor);
        if ($result === SUCCESS) {
    ...
    private function edit()
    {
        ...
        $name = htmlspecialchars(strip_tags($_POST['name']));
        $status = intval($_POST['status']);
        $checkExist = $this->joke_category_model->get($id);
        if (empty($checkExist)) {
            $response = json_encode(['errCode' => CURSE_CATEGORY_NOT_EXIST]);
            echo $response;
        }
        $result = $this->joke_category_model->edit($id, $name, $status);
        if ($result === SUCCESS) {
            $response = json_encode(['errCode' => SUCCESS, 'redirect' => 'list.php']);
            echo $response;
            exit;
    ...
    {
        $id = intval($_POST['id']);
        $result = $this->joke_category_model->delete($id);
        if ($result === SUCCESS) {
            $response = json_encode(['errCode' => SUCCESS, 'redirect' => 'list.php']);
            echo $response;
            exit;
...
$joke_category = new Joke_category(new Joke_category_model($db), new Account_model($db));

if (in_array('export', $query_array)) {
    $joke_category->export($query_array[1]);
    exit;
}

Util::requestEntry(object: $joke_category);
